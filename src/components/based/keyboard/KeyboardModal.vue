                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h1 class="modal-title fs-5" id="keyboardModalLabel">
                            ...
                            {{ langSet[lang || 'en'].keyboard.fullTitle || 'Virtual Keyboard' }}
                        </h1>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body py-3 py-md-4">
                        <KeyboardSetComp :class="[!isHowto ? '': 'd-none']" :lang="lang || 'en'" />
                        <CompKeyboardGuide :class="[isHowto ? '': 'd-none']" :lang="lang || 'en'" />
                    </div>
                    <div class="modal-footer">
                        <div v-if="!isHowto">
                            <span class="text-muted me-1">{{ langSet[lang || 'en'].keyboard.gettingLost }}</span>
                            <button class="btn btn-sm btn-outline-primary" @click="showHowTo">
                                <i class="bi bi-info-circle"></i>
                                {{ langSet[lang || 'en'].keyboard.howToUse }}
                            </button>
                        </div>
                         <div v-if="isHowto">
                            <button class="btn btn-outline-light bg-fabulous" @click="showKeyboard">
                                <i class="bi bi-keyboard"></i>
                                {{ langSet[lang || 'en'].keyboard.switchToKeyboard }}
                            </button>
                        </div>
                    </div>
                </div>
