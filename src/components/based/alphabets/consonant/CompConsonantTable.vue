<template>
    <div id="consonant-table" class="consonant-table" style="--bs-breadcrumb-divider: '>';">
        <h2>{{ langSet[lang || 'en'].learnAlphabets.consonants }} (35)</h2>
        <p class="text-dark" v-html="langSet[lang || 'en'].consonantView.description"></p>

        <div class="mt-4 d-flex">
            <div style="width: 24px; height: 24px;" class="bg-secondary"></div>
            <span style="margin-left: 8px;">
                = {{ langSet[lang || 'en'].consonantView.zeroConsonants }}
            </span>
            <div class="ms-1">
                <span class="ms-2 badge rounded-pill text-bg-light text-primary fw-bold">CL</span>
                <span>= {{ langSet[lang || 'en'].consonantView.clearConsonants }}</span>
            </div>
            <div class="ms-1">
                <span class="ms-2 badge rounded-pill text-bg-light text-danger fw-bold">BT</span>
                <span>= {{ langSet[lang || 'en'].consonantView.breathyConsonants }}</span>
            </div>
        </div>
        <table class="my-2">
            <thead>
            ...
            <tbody>
                <tr>
                    ...
                    <td class="bg-light">{{ langSet[lang || 'en'].consonantView.column }} 1</td>
                    <td class="bg-light">{{ langSet[lang || 'en'].consonantView.column }} 2</td>
                    <td class="bg-light">{{ langSet[lang || 'en'].consonantView.column }} 3</td>
                    <td class="bg-light">{{ langSet[lang || 'en'].consonantView.column }} 4</td>
                    <td class="bg-light">{{ langSet[lang || 'en'].consonantView.column }} 5</td>
                </tr>
                <tr v-for="(item, index) in consonants" :key="index">
                    <td :class="item.type === 'none' ? 'bg-secondary' : 'bg-light'">
                        <span>{{ index < 5 ? `${langSet[lang || 'en'].consonantView.row} ${index + 1}` : '-'
                                }}</span>
                    </td>
        ...
        <p>{{ langSet[lang || 'en'].consonantView.note }}</p>
    </div>
</template>
